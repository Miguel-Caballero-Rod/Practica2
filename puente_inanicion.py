...
class Monitor():
    def __init__(self):
        ...
        self.esperando_norte = Value('i', 0)
        self.esperando_sur = Value('i', 0)
        self.esperando_pedestrians = Value('i', 0)
        self.turn = Value('i', -1)
        # 0 for north cars
        # 1 for south cars
        # 2 for pedesrians
    ...
    def free_for_north(self) -> bool:
        return self.no_pedestrians() and self.nobody_from_south() and (self.turn.value == -1 or self.turn.value == 0)
    
    def free_for_south(self) -> bool:
        return self.nobody_from_north() and self.no_pedestrians() and (self.turn.value == -1 or self.turn.value == 1)
    
    def free_for_pedestrians(self) -> bool:
        return self.nobody_from_north() and self.nobody_from_south() and (self.turn.value == -1 or self.turn.value == 2)

    def wants_enter_car(self, direction: int) -> None:
        self.mutex.acquire()
        self.num_monitor.value += 1
        if direction == 1:
            self.esperando_sur.value += 1
            self.pueden_entrar_sur.wait_for(self.free_for_south)
            self.turn.value = 1
            self.num_sur.value += 1
            self.esperando_sur.value -= 1
        else:
            self.esperando_norte.value += 1
    ...
    def leaves_car(self, direction: int) -> None:
        ...
        if direction == 1:
            ...
            self.num_norte.value -= 1
        self.turn.value = -1
        if self.esperando_sur.value >= 5:
            self.turn.value = 1
            self.pueden_entrar_sur.notify_all()
        if self.esperando_norte.value >= 5:
            self.turn.value = 0
            self.pueden_entrar_norte.notify_all()
        if self.esperando_pedestrians.value >= 5:
            self.turn.value = 2
            self.pueden_entrar_pedestrians.notify_all()
        if direction == 1 and self.num_sur.value == 0:
            self.pueden_entrar_sur.notify_all()
            self.pueden_entrar_norte.notify_all()
            self.pueden_entrar_pedestrians.notify_all()
        elif direction == 0 and self.num_norte.value == 0:
            self.pueden_entrar_norte.notify_all()
            self.pueden_entrar_pedestrians.notify_all()
            self.pueden_entrar_sur.notify_all()
        self.mutex.release()
    ...
    def wants_enter_pedestrian(self) -> None:
        self.mutex.acquire()
        self.num_monitor.value += 1
        self.esperando_pedestrians.value += 1
        self.pueden_entrar_pedestrians.wait_for(self.free_for_pedestrians)
        self.turn.value = 2
        self.esperando_pedestrians.value -= 1
    ...
    def leaves_pedestrian(self) -> None:
        ...
        if self.esperando_pedestrians.value >= 5:
            self.turn.value = 2
            self.pueden_entrar_pedestrians.notify_all()
        if self.num_pedestrians.value == 0:
            self.pueden_entrar_pedestrians.notify_all()
            self.pueden_entrar_sur.notify_all()
            self.pueden_entrar_norte.notify_all()
        self.mutex.release()
